...
$("#loginForm").on("submit", function (e) {
    ...
    with (this) {
        ...
        if (isValidData) {
            $("#loginBtn").html(
                '<div class="spinner-border spinner-border-sm text-light" role="status"><span class="visually-hidden">Loading...</span></div>Login'
            );
            $.ajax({
                method: "POST",
                url: SITE_URL + "/view/admin/handle_login.php",
                data: $("#loginForm").serialize(),
                success: function (result) {
                    if (result === "admin loggedin") {
                        window.location = SITE_URL + "view/admin/index.php";
                        return;
                    }
                    $("#errorMsg").show();
